...
- ✅ **KDV-MOBILE-05**: Feed pagination uses 44px touch arrows (with optional swipe), the header is sticky with the logo while scrolling down, and a back-to-feed FAB sits bottom-right on dump pages. *(§6.5; declarative engine pagination — page 1 SSR at the root, separate page files kept reachable; swipe not implemented (optional))*. The masthead reflows PROGRESSIVELY by available room, content-driven, with NO media/container breakpoints: the logo mark, species chip and nav share ONE wrapping left cluster (`.masthead-main`, `flex:1 1 auto; min-width:0`), while the language switcher is a `flex:none` SIBLING of the cluster — so it ALWAYS shares row 1 with the mark at the far right, at every width and for long CJK strings. As room allows the chip and then the nav climb onto that first row (order: chip → nav; visual order mark · (chip) · (nav) …switcher): VERY NARROW = row1 mark + switcher, row2 chip (only when a declaration exists), row3 nav; MEDIUM = row1 mark + chip + switcher, row2 nav; WIDE = row1 mark + chip + nav + switcher, with the nav flushed right against the switcher by a presentational, zero-base grow-spacer (`.masthead-spacer{flex:1 1 0;min-width:0}` between the cluster and the nav) — it never forces a wrap, so a WRAPPED nav keeps its left alignment (never `.nav{margin-left:auto}`, which would right-align the lone nav too). The empty species group is removed from the flow (`.masthead-main .species-declaration:has(.species-chip[hidden]){display:none}`), so a visitor without a declaration gets no empty row and no phantom gap; the old `@container masthead (max-width:640px)` rule, its `container-type`, and the `@media (max-width:720px)` masthead reflow are removed. The switcher's 44px tap box is nudged by a relative `top:calc((var(--mark-size) - var(--tap)) / 2)` so its VISUAL centre lands on the mark's centre (top-edge sharing alone would sag by 5px); `--mark-size` is the single, component-scoped mark-box token and the popover stays anchored directly under the control.
