...
- ✅ **KDV-CONTRACT-01**: `/index.json` is served with `platform`, `version`, `generated_at`, `base_url`, `total`, the whole §5.2 BIOS embedded verbatim as `protocol`, and a `dumps` entry carrying the §5.1 per-dump fields including `body_url` (the raw markdown to download, not the HTML page). *(§5.1)*
...
- ✅ **KDV-CONTRACT-05**: `/.well-known/kodavr.json` is served with platform/version/description, the endpoints map (including `body_pattern`), the submission block, the consumption contract, the self-explaining BIOS (`about`/`index_url`/`interpret`/`how_to_consume`) and the trust/stakes/flags vocabularies; this exact object is embedded as `protocol` in index.json and the published manifest. *(§5.2)*
...
- ✅ **KDV-SURFACE-14**: The prompt is pinned to the page: a dump page's prompt hardcodes that dump's own `body_url` (raw markdown, not the HTML page) and `manifest_url` and may point the agent at `/index.json` for more, while `/reception/` keeps the universal index prompt. *(§7.4, §7.11)*
