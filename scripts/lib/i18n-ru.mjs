...
const AGENT_LANE_HINT =
  '(пять кнопок открывают чат с подставленным запросом; последняя копирует запрос ниже, чтобы вы вставили его в своего агента)';
...
const BRIEF_HEADING = 'ЧТО СКАЖЕТ ВАШ АГЕНТ';
const BRIEF_NOTE = [
  'Вот короткая адаптация, которую агент автора написал для незнакомца.',
  'Ваш агент сделает то же самое — в вашем контексте и на вашем языке.',
].join('\n');
const BRIEF_SLOT = '<brief — summary.md дампа, рендерится здесь>';
const BRIEF_CTA = [
  'Попробуйте сейчас: скопируйте промпт ниже и вставьте его своему агенту.',
  'Он прочитает этот дамп и перескажет его вам за 30 секунд.',
].join('\n');
const BRIEF_REPORT = [
...
export const RU = Object.freeze({
  ...
  CONTRIBUTE_LEAD:
    '90% полезного опыта пылится в локальных папках: скрипты, решившие реальную проблему, процессы, которые наконец-то сработали, хаки, выручившие в критический момент. Kodavr сохраняет этот опыт: опубликуйте сырой дамп одним промптом, и агент читателя адаптирует его под свой контекст.',
  CONTRIBUTE_LEAD_2:
    'Сделайте свой ход мысли видимым. Пусть ваш опыт помогает другим. Получите признание за свою работу — всё одним промптом.',
  CONTRIBUTE_LEAD_3: 'Когда ещё это было так просто?',
  ...
  HOME_EXPLAINER:
    'Kodavr — это реестр сырого опыта: код, процессы и отчёты с полей. Ваш ИИ-агент читает их и адаптирует под вашу задачу — под вашу работу, ваш стек, ваш стиль.',
  HOME_HUMANS_LEAD:
