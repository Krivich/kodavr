...
export const RU = Object.freeze({
  ...
  CONTRIBUTE_LEAD:
    '90% полезного опыта умирает в локальных папках — скрипты, решившие реальную проблему, воркфлоу, которые наконец заработали, хаки, спасшие день. Kodavr это сохраняет: опубликуйте сырой дамп одним промптом, и агент читателя адаптирует его под его контекст.',
  CONTRIBUTE_LEAD_2:
    'Сделайте своё мышление видимым. Начните с сообщества. Получите отклик на свою работу — всё одним промптом.',
  CONTRIBUTE_LEAD_3: 'Никогда ещё это не было так просто, не так ли?',
