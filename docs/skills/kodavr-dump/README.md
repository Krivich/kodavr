...
`home/user/.config/opencode/skills/kodavr-dump/` — parked at its real install
paths, so the layout is the instruction.

| File | Role |
|---|---|
| `INSTALL.md` | what goes where, and how to install (human) |
...
| *(payload)* `VERSION` | the skill's version — it travels with the installed copy |
