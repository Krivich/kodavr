...
```
Requirements: dumps sorted by date descending; each dump's manifest is additionally published as a separate `manifest.json` file in the dump's directory (stable URL); `commit_sha` present in the full manifest (injected at build time).

### 5.2 `/.well-known/kodavr.json` — discovery protocol
```json
{
  "platform": "kodavr",
  "version": "1.0",
