...
```json
{
  ...
  "protocol": "injected-at-build-time (the whole §5.2 BIOS document, embedded verbatim)",
...
The whole §5.2 protocol document (the machine BIOS) is embedded verbatim under `protocol`, so a machine sees where it is whichever door it entered. Nothing is repeated flat — one source.
```json
{
  "platform": "kodavr",
  "version": "1.0",
  ...
  "total": 2,
  "protocol": { "...": "the whole §5.2 document, embedded verbatim" },
  "dumps": [
    {
      "slug": "...",
...
```
Requirements: dumps sorted by date descending; each dump's manifest is additionally published as a separate `manifest.json` file in the dump's directory (stable URL); every layer file listed in that manifest is published beside it at `/dumps/<slug>/<file>` (so `body_url`, the raw layer, is downloadable markdown — never the HTML projection); `commit_sha` present in the full manifest (injected at build time). `index.json` embeds the protocol in place (`protocol`, the §5.2 BIOS): a consuming agent holds this document already, so the contract must not live only behind a link. The published manifest embeds the same `protocol` object.

### 5.2 `/.well-known/kodavr.json` — discovery protocol and machine BIOS
This document is the single machine BIOS: it is served standalone here and embedded verbatim as `protocol` in `index.json` and every published manifest. It must explain itself to the dumbest agent — where it is (`about`), what to do with a dump's metadata (`interpret`) and how to consume the collection (`how_to_consume`) — in addition to the discovery/publication fields.
```json
{
  "platform": "kodavr",
  "version": "1.0",
...
The platform-level prompt, shown once on `/reception/`. A dump page uses §7.11 instead. It points the agent at `index.json`'s own `how_to_consume` block (§5.1), so the instruction is in the document the agent already holds.
```
...
A shared dump link must hand the agent THAT dump, not the whole platform; the index is offered as an optional follow-up. The agent is given the raw markdown and the manifest, never the HTML projection. `<body-url>` (the raw layer, §5.1 `body_url`), `<manifest-url>` and `<index-url>` are the page's own absolute URLs.
```
Read the dump's raw markdown at <body-url> (manifest: <manifest-url>).
...
where needed, warn me or refuse to retell. If it fits, also check <index-url>
